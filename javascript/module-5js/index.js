...
//const getUsersWithAge = (arr, min, max) => arr.sort((user) => user.age > min).sort((user) => user.age < max);
 ...
 */
const getUsersByFriend = (arr, name) => arr.filter((x) => x.friends.includes(name)).map((user) => user.name);
